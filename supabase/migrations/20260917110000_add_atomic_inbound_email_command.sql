...
begin
  ...
  if new.type = 'system'
     and new.metadata->>'action' = 'archived' then
    return new;
  end if;

  -- External reality can continue after a booking has been archived. Preserve an
  -- inbound provider email without reopening or mutating the booking itself.
  -- Only the trusted service role may use this exception; authenticated users
  -- remain unable to append arbitrary Activity to archived bookings.
  if current_user = 'service_role'
     and new.type = 'email'
     and new.direction = 'inbound'
     and new.metadata->>'ingested_by' = 'brevo_inbound' then
    return new;
  end if;
...
begin
  ...
  insert into public.email_messages (
    ...
    target_contact_id,
    'inbound',
    trim(target_to_email),
    lower(trim(target_from_email)),
    trim(target_subject),
    trim(target_body_text),
    'received',
    lower(trim(target_provider)),
  ...
  if resolved_email_id is null then
    raise exception 'inbound_email_resolution_failed';
  end if;
  ...
  return query
  select resolved_email_id, did_create_email, did_create_activity;
end;
$$;

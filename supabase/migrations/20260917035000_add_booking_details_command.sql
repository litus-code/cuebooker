...
create or replace function public.update_booking_details(
  ...
  next_start_time time default null,
  next_end_time time default null,
...
begin
  ...
  if next_start_time is not null and next_end_time is not null and next_end_time <= next_start_time then
    raise exception 'invalid_booking_time_range';
  end if;
...
comment on function public.update_booking_details(
  uuid, uuid, text, text, text, text, date, time, time, text, bigint, text, text
) is 'Atomically replaces editable Booking Core event/commercial details and appends a system Activity containing the changed field names.';

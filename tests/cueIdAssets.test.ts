...
test('original Club Minimal candidate remains outside production catalogue until approved', () => {
  ...
  assert.equal(CUE_ID_CANDIDATE_ASSET.compressedBytes, 61_196)
  assert.equal(CUE_ID_CANDIDATE_ASSET.triangles, 3_976)
...
test('Club Minimal candidate reports substantial quality headroom', () => {
  ...
  assert.equal(headroom.bytesRemaining, 938_804)
  assert.equal(headroom.trianglesRemaining, 31_024)

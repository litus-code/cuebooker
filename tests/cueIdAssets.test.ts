...
test('original Club Minimal candidate remains outside production catalogue until approved', () => {
  ...
  assert.equal(CUE_ID_CANDIDATE_ASSET.compressedBytes, 64_196)
  assert.equal(CUE_ID_CANDIDATE_ASSET.triangles, 4_240)
...
test('Club Minimal candidate reports substantial quality headroom', () => {
  ...
  assert.equal(headroom.bytesRemaining, 935_804)
  assert.equal(headroom.trianglesRemaining, 30_760)
...
test('Club Minimal quality ladder stays inside the universal base budget', () => {
  assert.deepEqual(
    ...
    {
      light: { bytes: 64_196, triangles: 4_240 },
      medium: { bytes: 104_456, triangles: 8_816 },
      high: { bytes: 194_160, triangles: 20_000 }
    }
  )

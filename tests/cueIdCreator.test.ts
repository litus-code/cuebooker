...
test('CUE ID creator UI uses the creator config and runtime adapter', async () => {
  ...
  assert.match(component, /creator__mobile-tabs/)
  assert.match(component, /v-for="\(step, index\) in steps"/)
  assert.match(component, /:aria-current="activeStep === step\.id \? 'step' : undefined"/)
  assert.match(component, /pending: !visualCoverage\[step\.id\]/)
  ...
  assert.match(component, /authoredLabManifest/)
  assert.match(component, /resolveCueIdCreator3dLabCandidate/)
  assert.match(component, /:creator-config="modelValue"/)
  assert.match(component, /:lab-authored-manifest="authoredLabManifest"/)
  ...
  assert.match(component, /:show-placeholder-figure="!creatorUsesLabFixture && !authoredLabManifest"/)

...
test('CUE ID creator UI uses the creator config and runtime adapter', async () => {
  ...
  assert.match(component, /CUE_ID_PRODUCTION_CATALOGUE/)
  assert.match(component, /getCueIdCreatorAssetStatus/)
  assert.match(component, /creatorUsesLabFixture/)
  assert.match(component, /creator__asset-status/)
  assert.match(component, /gatePerformance/)
  assert.match(component, /:lab-asset="creatorUsesLabFixture \? 'candidate' : null"/)

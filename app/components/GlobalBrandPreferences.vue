...
<template>
  <header v-if="!isHome" class="global-brand-preferences" :class="{ 'global-brand-preferences--controls-only': isWorkspacePrototype }">
    <NuxtLink v-if="!isWorkspacePrototype" class="global-brand-preferences__brand" to="/" aria-label="Cuebooker"><img src="/cuebooker-mark.svg" alt="" /><span>uebooker</span></NuxtLink>
...
.global-brand-preferences { align-items:center; display:flex; justify-content:space-between; min-height:64px; padding:10px 24px; position:relative; z-index:40; }
.global-brand-preferences__brand { align-items:center; color:var(--cue-text,#f2f0eb); display:inline-flex; font-size:14px; font-weight:900; gap:8px; letter-spacing:.04em; text-decoration:none; }
...
@media (max-width:720px) { .global-brand-preferences { min-height:58px; padding:8px 14px; } .global-brand-preferences__brand { font-size:12px; } .global-brand-preferences__brand img { height:34px; width:34px; } .global-brand-preferences--controls-only { padding:0; right:10px; top:10px; } }
</style>

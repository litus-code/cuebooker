...
const archiveView = ref<'inbox' | 'archived'>('inbox')
...
const copy = computed(() => props.locale === 'es' ? {
  ...
  inbox: 'Inbox', archived: 'Archived', archive: 'Archive', restore: 'Restore', archiveHint: 'Inbox keeps everything you still want at hand, whether live or already decided. Archiving removes it from day-to-day operations without deleting its history.', archivedReadOnly: 'Archived booking. Its trace is preserved in read-only mode.'
})
...
const archiveScopedBookings = computed(() => {
  ...
  return props.bookings.filter(booking => {
    if (archiveView.value === 'inbox' && booking.archived_at) return false
...
watch(
  ...
  ([value]) => {
    ...
    archiveView.value = booking.archived_at ? 'archived' : 'inbox'
  ...
  <section class="core-inbox">
    ...
    <div v-if="bookings.length" id="core-inbox-tools" class="core-inbox__tools">
      ...
      <div class="core-inbox__filters core-inbox__filters--archive" :title="copy.archiveHint">
        <button type="button" :class="{ active: archiveView === 'inbox' }" @click="archiveView = 'inbox'">{{ copy.inbox }} · {{ bookings.filter(item => !item.archived_at).length }}</button>
    ...
    <div v-if="bookings.length && !visibleBookings.length" class="core-inbox__empty core-inbox__empty--filtered">
      ...
      <button v-else-if="archiveView !== 'inbox'" type="button" @click="archiveView = 'inbox'">
        {{ locale === 'es' ? 'Volver al Inbox' : 'Back to Inbox' }}
      </button>
    </div>

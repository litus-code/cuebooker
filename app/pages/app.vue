...
<template>
  <main class="workspace-page">
      ...
      <div class="booking-workspace">
        ...
        <article v-if="selected" class="booking-detail">
          ...
          <div class="booking-facts">
            <section><h3>{{ copy.event }}</h3><dl><div><dt>Fecha</dt><dd>{{ formatDate(selected.event.date) }}</dd></div><div><dt>Ciudad</dt><dd>{{ selected.event.city }}</dd></div><div><dt>Sala</dt><dd>{{ selected.event.venue }}</dd></div><div><dt>Aforo</dt><dd>{{ selected.event.capacity }}</dd></div><div><dt>Oferta</dt><dd>{{ selected.event.offer }}</dd></div><div><dt>Horario</dt><dd>{{ selected.event.schedule || '—' }}</dd></div></dl></section>
            <section><h3>{{ copy.contact }}</h3><dl><div><dt>Nombre</dt><dd>{{ selected.promoter.name }}</dd></div><div><dt>Email</dt><dd>{{ selected.promoter.email }}</dd></div><div v-if="selected.promoter.phone"><dt>Tel.</dt><dd>{{ selected.promoter.phone }}</dd></div><div><dt>Origen</dt><dd>Booking link</dd></div></dl></section>
          </div>
    ...
    <section v-else-if="activeView === 'calendar'" class="workspace-panel"><p class="eyebrow">CALENDAR / PRIVATE</p><h2>{{ copy.calendarTitle }}</h2><p>{{ copy.calendarBody }}</p><div class="calendar-list"><article v-for="booking in confirmedBookings" :key="booking.id"><time>{{ formatDate(booking.event.date) }}</time><strong>{{ booking.event.venue }}</strong><span>{{ booking.event.city }} · {{ booking.artistName }}</span></article></div></section>

    <section v-else-if="activeView === 'history'" class="workspace-panel"><p class="eyebrow">ARCHIVE / REVERSIBLE</p><h2>{{ copy.historyTitle }}</h2><p>{{ copy.historyBody }}</p><div class="history-list"><article v-for="booking in historyBookings" :key="booking.id"><div><strong>{{ booking.event.venue }}</strong><span>{{ formatDate(booking.event.date) }} · {{ booking.artistName }}</span></div><NuxtLink :to="`/request?id=${booking.id}`">{{ copy.promoterView }}</NuxtLink><button @click="setArchived(booking.id, false)">{{ copy.restore }}</button></article></div></section>

    <section v-else class="workspace-panel"><p class="eyebrow">DEVICE / PREFERENCES</p><h2>{{ copy.settingsTitle }}</h2><p>{{ copy.settingsBody }}</p><div class="settings-row"><span>Idioma</span><div class="locale-control"><button :class="{ active: locale === 'es' }" @click="setLocale('es')">ES</button><button :class="{ active: locale === 'en' }" @click="setLocale('en')">EN</button></div></div><div class="settings-row"><span>Apariencia</span><div class="theme-control"><button v-for="value in ['dark', 'light'] as const" :key="value" :class="{ active: theme === value }" @click="setTheme(value)">{{ value.toUpperCase() }}</button></div></div></section>

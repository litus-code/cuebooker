...
const copy = computed(() => preferences.locale.value === 'es' ? {
  ...
  title: 'CUE ID, CONSTRUYE TU PRESENCIA.',
  body: 'Esta primera base prueba una identidad 3D humana y modular. Empezamos por Club Minimal para demostrar que CUE ID puede sentirse cercano, real y de club sin obligar a nadie a entrar en una estética futurista.',
  noteTitle: 'Qué estamos validando',
  noteBody: 'Identificación, proporciones, outfit, actitud e interacción, manteniendo el coste del 3D fuera de la carga inicial del perfil. Después de validar esta base produciremos assets 3D con más detalle.',
  returnHome: 'Volver'
} : {
  eyebrow: 'LAB / ARTIST PROFILE',
  title: 'CUE ID, BUILD YOUR PRESENCE.',
  body: 'This first base tests a human, modular 3D identity. We start with Club Minimal to prove CUE ID can feel grounded, real and rooted in club culture without forcing everyone into a futuristic aesthetic.',
  noteTitle: 'What we are validating',
  noteBody: 'Identification, proportions, outfit, attitude and interaction while keeping the 3D cost outside the initial profile load. Once this base is validated, more detailed 3D assets can follow.',
  returnHome: 'Back'
})
...
<template>
  <main class="cue-id-page">
    ...
    <section class="cue-id-page__note">
      <span>IDENTITY SYSTEM / 01</span>
      <div>
        <h2>{{ copy.noteTitle }}</h2>
        <p>{{ copy.noteBody }}</p>
      </div>
      <small>HUMAN BASE → MODULAR LOOK → OPTIONAL 3D</small>
    </section>
  </main>
</template>
...
.cue-id-page__intro h1 { max-width: 1050px; margin: 0; font-size: clamp(3rem,7.4vw,7.4rem); line-height: .82; letter-spacing: -.06em; text-transform: uppercase; }
.cue-id-page__intro span { display: block; max-width: 760px; margin-top: 26px; color: var(--cue-muted); font-size: 15px; line-height: 1.65; }

...
const copy = computed(() => locale.value === 'es' ? {
  kicker: 'CONFIGURACIÓN DE CUENTA / 01', title: '¿Cómo trabajas?', body: 'Esto define la identidad privada que administrará calendario y bookings.',
  dj: 'DJ / ARTISTA', djBody: 'Gestiono mi propio proyecto y calendario.', agency: 'AGENCIA', agencyBody: 'Gestiono un roster y su operativa de booking.',
  yourName: 'Tu nombre', artistName: 'Nombre artístico', agencyName: 'Nombre de la agencia', slug: 'Identificador', saving: 'Guardando…', create: 'Crear workspace', error: 'No se pudo completar la configuración.'
} : {
  kicker: 'ACCOUNT SETUP / 01', title: 'How do you work?', body: 'This defines the private identity that will manage calendars and bookings.',
  dj: 'DJ / ARTIST', djBody: 'I manage my own project and calendar.', agency: 'AGENCY', agencyBody: 'I manage a roster and its booking operations.',
  yourName: 'Your name', artistName: 'Artist name', agencyName: 'Agency name', slug: 'Identifier', saving: 'Saving…', create: 'Create workspace', error: 'Setup could not be completed.'
})
...
async function submit() {
  ...
  try {
    ...
    errorMessage.value = error?.data?.message || error?.message || copy.value.error
  } finally {
    submitting.value = false
  }
}

useHead(() => ({ title: locale.value === 'es' ? 'Configura tu cuenta | CueBooker' : 'Set up your account | CueBooker', htmlAttrs: { lang: locale.value } }))
</script>

<template>
  <main class="onboarding-page">
    <header class="onboarding-header"><NuxtLink class="onboarding-brand" to="/">CUEBOOKER<span>/</span></NuxtLink><CuePreferencesControl compact /></header>
    ...
    <section class="onboarding-panel">
      ...
      <form class="onboarding-form" @submit.prevent="submit">
        ...
        <label>
          <span>{{ accountType === 'dj' ? copy.artistName : copy.agencyName }}</span>
        ...
        <button class="onboarding-submit" type="submit" :disabled="submitting">
          {{ submitting ? copy.saving : copy.create }}
        </button>
      </form>
    </section>
  </main>
...
.onboarding-brand span { color: var(--cue-toggle); }
...
.type-grid button.active { border-color: var(--cue-toggle); box-shadow: inset 0 0 0 1px var(--cue-toggle); }
...
.onboarding-form { display: grid; gap: 16px; padding: 26px; border: 1px solid var(--cue-border); background: var(--cue-surface); }
...
.onboarding-submit { min-height: 50px; border: 0; background: var(--cue-toggle); color: #070707; font-weight: 800; cursor: pointer; }

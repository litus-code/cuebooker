...
const text = computed(() => locale.value === 'es' ? {
  ...
  hello: 'Seguimiento de tu solicitud', intro: 'No necesitas instalar una app. Aquí puedes leer la respuesta del DJ, contestar y confirmar la fecha.', summary: 'Resumen', date: 'Fecha', venue: 'Sala', city: 'Ciudad', offer: 'Oferta', conversation: 'Conversación', reply: 'Responder', placeholder: 'Escribe tu respuesta…', send: 'Enviar respuesta', confirm: 'Aceptar y confirmar fecha', confirmed: 'Fecha confirmada', rejected: 'Solicitud rechazada', email: 'En el producto real llegarías aquí desde el botón incluido en el email de CueBooker.'
} : {
...
<template>
  <main class="promoter-page">
    <header class="profile-nav"><NuxtLink to="/">CUEBOOKER<span>/</span></NuxtLink><p>{{ text.demo }}</p><CuePreferencesControl compact /></header>
    ...
    <template v-else>
      ...
      <section class="promoter-booking">
        <article class="promoter-summary"><p class="eyebrow">{{ text.summary }}</p><h2>{{ booking.event.name || booking.event.venue }}</h2><dl><div><dt>DJ</dt><dd>{{ booking.artistName }}</dd></div><div><dt>{{ text.date }}</dt><dd>{{ formatDate(booking.event.date) }}</dd></div><div><dt>{{ text.venue }}</dt><dd>{{ booking.event.venue }}</dd></div><div><dt>{{ text.city }}</dt><dd>{{ booking.event.city }}</dd></div><div><dt>{{ text.offer }}</dt><dd>{{ booking.event.offer }}</dd></div></dl><strong v-if="booking.status === 'confirmed'" class="confirmed-label">✓ {{ text.confirmed }}</strong><strong v-else-if="booking.status === 'rejected'" class="rejected-label">{{ text.rejected }}</strong><button v-else class="button button--primary" @click="setStatus(booking.id, 'confirmed')">{{ text.confirm }} <span>✓</span></button></article>

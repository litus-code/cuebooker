...
<template>
  <main class="promoter-page">
    ...
    <section v-else-if="!booking" class="promoter-state"><p>{{ text.missing }}</p><NuxtLink class="button button--primary" to="/artist">{{ text.back }} <span>↗</span></NuxtLink></section>
    <template v-else>
      ...
      <section class="promoter-booking">
        <article class="promoter-summary"><p class="eyebrow">{{ text.summary }}</p><h2>{{ booking.event.name || booking.event.venue }}</h2><dl><div><dt>DJ</dt><dd>{{ booking.artistName }}</dd></div><div><dt>Fecha</dt><dd>{{ formatDate(booking.event.date) }}</dd></div><div><dt>Sala</dt><dd>{{ booking.event.venue }}</dd></div><div><dt>Ciudad</dt><dd>{{ booking.event.city }}</dd></div><div><dt>Oferta</dt><dd>{{ booking.event.offer }}</dd></div></dl><strong v-if="booking.status === 'confirmed'" class="confirmed-label">✓ {{ text.confirmed }}</strong><button v-else class="button button--primary" @click="setStatus(booking.id, 'confirmed')">{{ text.confirm }} <span>✓</span></button></article>
        <article class="promoter-thread"><p class="eyebrow">{{ text.conversation }}</p><div class="message-thread"><article v-for="message in booking.messages" :key="message.id" :class="`message message--${message.actor}`"><header><strong>{{ message.actor === 'artist' ? booking.artistName : booking.promoter.name }}</strong><time>{{ new Date(message.createdAt).toLocaleString(locale === 'es' ? 'es-ES' : 'en-GB') }}</time></header><p>{{ message.body }}</p><a v-for="file in message.attachments" :key="file.id" href="#" @click.prevent>↳ {{ file.name }}</a></article></div><form class="booking-reply" @submit.prevent="sendReply"><label>{{ text.reply }}<textarea v-model="reply" rows="5" :placeholder="text.placeholder" /></label><button class="button button--primary" :disabled="!reply.trim()">{{ text.send }} <span>↗</span></button></form></article>
      </section>
    </template>
  </main>
</template>

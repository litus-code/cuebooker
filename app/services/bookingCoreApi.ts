...
export function createBookingCoreApi(options: BookingCoreApiOptions) {
  ...
  async function listArtistBookingsForDate(
    ...
    if (!workspaceId || !artistId || !eventDate) return [] as CoreBooking[]
    return $fetch<CoreBooking[]>(`${baseUrl}/rest/v1/bookings`, {
      headers: authHeaders(),
      query: {
        workspace_id: `eq.${workspaceId}`,
  ...
  async function listBookingEmailMessages(workspaceId: string, bookingId: string) {
    return $fetch<BookingEmailMessage[]>(`${baseUrl}/rest/v1/email_messages`, {
      ...
      query: {
        workspace_id: `eq.${workspaceId}`,
        booking_id: `eq.${bookingId}`,
        select: 'id,booking_id,to_email,delivery_status,delivered_at,bounced_at,opened_at,last_delivery_event_at,delivery_failure_code,created_at',
        order: 'created_at.asc',
        limit: '100'
      }
    })
